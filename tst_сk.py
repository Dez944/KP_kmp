...
def key_act():
    if key.get() == '1':
        ...
        messagebox.showerror(title='Активация первана', message='Введите корректный ключ')
...
actK = Toplevel()
actK.title('Activation')
key = Entry(actK, width=80)
key.grid()
button = Button(actK, text='Ввести', command=key_act).grid()
